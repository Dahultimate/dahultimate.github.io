...
>
> **Changement (mise en cache)** : `services/events.repository.ts::fetchEvents()` met désormais les événements en cache en mémoire pendant **15 minutes** (évite de refaire l'aller-retour réseau à chaque changement d'écran — accueil, liste, détail). Le cache est vidé automatiquement dès qu'un événement est créé ou modifié (`createEvent`/`updateEvent`), donc l'auteur d'un changement le voit toujours immédiatement ; les autres membres peuvent voir les anciennes données jusqu'à 15 minutes avant de se rafraîchir (recharger la page force aussi un rafraîchissement, le cache est en mémoire, pas persistant).
